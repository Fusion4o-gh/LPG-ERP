...
import { deleteVendorOpeningBalance, updateVendorOpeningBalance } from "../../../../../server/services/opening-balances/opening-balances.ts";
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const opening = await updateVendorOpeningBalance(context, id, {
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const context = await getRequestContext(request);
    const { id } = await params;
    const result = await deleteVendorOpeningBalance(context, id);
    return ok(result);
  } catch (error) {
