...
export function CustomerOpeningBalanceManager() {
  const [customers, setCustomers] = useState<Record<string, unknown>[]>([]);
  const [rows, setRows] = useState<Record<string, unknown>[]>([]);
  const [values, setValues] = useState({ customerId: "", amount: "", balanceType: "DEBIT", transactionDate: today() });
  const [editingId, setEditingId] = useState("");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  ...
  async function load() {
    ...
    setError("");
    try {
      const data = await apiGet<{ openings: Record<string, unknown>[]; customers: Record<string, unknown>[] }>("/api/configuration/customer-opening-balance");
      setRows(data.openings);
      setCustomers(data.customers);
      if (!values.customerId && data.customers[0]?.id) setValues((current) => ({ ...current, customerId: String(data.customers[0].id) }));
    } catch (err) {
      setError(err instanceof Error ? err.message : "Load failed.");
    } finally {
      setLoading(false);
  ...
  function reset() {
    setValues({ customerId: customers[0]?.id ? String(customers[0].id) : "", amount: "", balanceType: "DEBIT", transactionDate: today() });
    setEditingId("");
  }

  function edit(row: Record<string, unknown>) {
    setEditingId(String(row.id));
    setValues({ customerId: String(row.customerId ?? ""), amount: String(row.amount ?? ""), balanceType: String(row.balanceType ?? "DEBIT"), transactionDate: dateValue(row.voucherDate) });
  }
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const payload = { ...values, amount: Number(values.amount) };
      if (editingId) await apiPut(`/api/configuration/customer-opening-balance/${editingId}`, payload);
      else await apiPost("/api/configuration/customer-opening-balance", payload);
      setSuccess(editingId ? "CustomerOpeningBalance updated." : "CustomerOpeningBalance created.");
      reset();
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Save failed.");
  ...
  async function remove(row: Record<string, unknown>) {
    if (!window.confirm("Delete this customer opening balance?")) return;
    setError("");
    setSuccess("");
    try {
      await apiDelete(`/api/configuration/customer-opening-balance/${row.id}`);
      setSuccess("CustomerOpeningBalance deleted.");
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Delete failed.");
    }
  ...
  return (
    <>
      <PageHeader title="CustomerOpeningBalance" description="Enter initial customer debit or credit balance before later customer movement is posted." />
      <div className="grid gap-5 xl:grid-cols-[minmax(320px,420px)_1fr]">
        <form onSubmit={submit} className="space-y-4 rounded-lg border border-blue-100 bg-white p-4 shadow-sm">
          <ApiError message={error} />
          <SuccessMessage message={success} />
          <FormSection title={editingId ? "Edit CustomerOpeningBalance" : "Add CustomerOpeningBalance"}>
            <div className="space-y-3">
              <Field label="Customer" required>
                <select value={values.customerId} onChange={(event) => setValues((current) => ({ ...current, customerId: event.target.value }))} className="w-full rounded-md border border-blue-200 bg-white px-3 py-2 focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100">
              ...
              <Field label="Amount" required>
                <input type="number" min="0.01" step="0.01" value={values.amount} onChange={(event) => setValues((current) => ({ ...current, amount: event.target.value }))} className="w-full rounded-md border border-blue-200 px-3 py-2 focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100" />
              </Field>
              <Field label="Debit / Credit Type" required>
                <select value={values.balanceType} onChange={(event) => setValues((current) => ({ ...current, balanceType: event.target.value }))} className="w-full rounded-md border border-blue-200 bg-white px-3 py-2 focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100">
                  <option value="DEBIT">Debit</option>
                  <option value="CREDIT">Credit</option>
                </select>
              </Field>
              <Field label="Date" required>
                <input type="date" value={values.transactionDate} onChange={(event) => setValues((current) => ({ ...current, transactionDate: event.target.value }))} className="w-full rounded-md border border-blue-200 px-3 py-2 focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100" />
              </Field>
            </div>
          </FormSection>
          <div className="flex gap-2">
            <button disabled={saving} className="rounded-md bg-blue-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">
              {saving ? "Saving..." : editingId ? "Update" : "Create"}
            </button>
            <button type="button" onClick={reset} className="rounded-md border border-blue-200 px-4 py-2 text-sm font-semibold text-blue-800">
              Reset
            </button>
          </div>
        </form>
        <DataTable
          ...
          columns={[
            ...
            { key: "balanceType", label: "Debit/Credit" },
            { key: "voucherNo", label: "Voucher No" },
            { key: "locked", label: "Status", render: (row) => (row.locked ? <span className="rounded bg-blue-100 px-2 py-1 text-xs font-semibold text-blue-900">Locked</span> : <span className="rounded bg-white px-2 py-1 text-xs font-semibold text-blue-700 ring-1 ring-blue-200">Open</span>) },
            {
              key: "actions",
              label: "Actions",
              render: (row) => (
                <div className="flex gap-2">
                  <button disabled={Boolean(row.locked)} onClick={() => edit(row)} className="rounded-md border border-blue-200 px-2 py-1 text-xs font-semibold text-blue-800 disabled:opacity-40">
                    Edit
                  </button>
                  <button disabled={Boolean(row.locked)} onClick={() => remove(row)} className="rounded-md border border-red-200 px-2 py-1 text-xs font-semibold text-red-700 disabled:opacity-40">
                    Delete
...
export function VendorOpeningBalanceManager() {
  const [vendors, setVendors] = useState<Record<string, unknown>[]>([]);
  const [rows, setRows] = useState<Record<string, unknown>[]>([]);
  const [values, setValues] = useState({ vendorId: "", amount: "", balanceType: "CREDIT", transactionDate: today() });
  const [editingId, setEditingId] = useState("");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  ...
  async function load() {
    ...
    setError("");
    try {
      const data = await apiGet<{ openings: Record<string, unknown>[]; vendors: Record<string, unknown>[] }>("/api/configuration/vendor-opening-balance");
      setRows(data.openings);
      setVendors(data.vendors);
      if (!values.vendorId && data.vendors[0]?.id) setValues((current) => ({ ...current, vendorId: String(data.vendors[0].id) }));
    } catch (err) {
      setError(err instanceof Error ? err.message : "Load failed.");
    } finally {
      setLoading(false);
  ...
  function reset() {
    setValues({ vendorId: vendors[0]?.id ? String(vendors[0].id) : "", amount: "", balanceType: "CREDIT", transactionDate: today() });
    setEditingId("");
  }

  function edit(row: Record<string, unknown>) {
    setEditingId(String(row.id));
    setValues({ vendorId: String(row.vendorId ?? ""), amount: String(row.amount ?? ""), balanceType: String(row.balanceType ?? "CREDIT"), transactionDate: dateValue(row.voucherDate) });
  }
  ...
  async function submit(event: FormEvent) {
    ...
    setError("");
    setSuccess("");
    if (!values.vendorId) return setError("Vendor is required.");
    if (!Number.isFinite(Number(values.amount)) || Number(values.amount) <= 0) return setError("Amount must be a positive number.");

    setSaving(true);
    try {
      const payload = { ...values, amount: Number(values.amount) };
      if (editingId) await apiPut(`/api/configuration/vendor-opening-balance/${editingId}`, payload);
      else await apiPost("/api/configuration/vendor-opening-balance", payload);
      setSuccess(editingId ? "Vendor Opening Balance updated." : "Vendor Opening Balance created.");
      reset();
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Save failed.");
  ...
  async function remove(row: Record<string, unknown>) {
    if (!window.confirm("Delete this vendor opening balance?")) return;
    setError("");
    setSuccess("");
    try {
      await apiDelete(`/api/configuration/vendor-opening-balance/${row.id}`);
      setSuccess("Vendor Opening Balance deleted.");
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Delete failed.");
    }
  ...
  return (
    <>
      <PageHeader title="Vendor Opening Balance" description="Enter initial vendor debit or credit balance before later vendor movement is posted." />
      <div className="grid gap-5 xl:grid-cols-[minmax(320px,420px)_1fr]">
        <form onSubmit={submit} className="space-y-4 rounded-lg border border-blue-100 bg-white p-4 shadow-sm">
          <ApiError message={error} />
          <SuccessMessage message={success} />
          <FormSection title={editingId ? "Edit Vendor Opening Balance" : "Add Vendor Opening Balance"}>
            <div className="space-y-3">
              <Field label="Vendor" required>
                <select value={values.vendorId} onChange={(event) => setValues((current) => ({ ...current, vendorId: event.target.value }))} className="w-full rounded-md border border-blue-200 bg-white px-3 py-2 focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100">
        ...
        <DataTable
          ...
          columns={[
            { key: "voucherDate", label: "Date", render: (row) => dateValue(row.voucherDate) },
            { key: "vendor", label: "Vendor", render: (row) => `${(row.vendor as { code?: string; name?: string })?.code ?? ""} ${(row.vendor as { name?: string })?.name ?? ""}` },
            { key: "amount", label: "Amount" },
            { key: "balanceType", label: "Debit/Credit" },
            { key: "voucherNo", label: "Voucher No" },
            { key: "locked", label: "Status", render: (row) => (row.locked ? <span className="rounded bg-blue-100 px-2 py-1 text-xs font-semibold text-blue-900">Locked</span> : <span className="rounded bg-white px-2 py-1 text-xs font-semibold text-blue-700 ring-1 ring-blue-200">Open</span>) },
