...
export function PaymentVoucherList({
  ...
  apiPath: string;
  addHref: string;
  reversalKind: "cash-payment" | "cash-receipt";
  typeLabel: string;
}) {
  ...
  async function load(nextPage = page, nextSearch = appliedSearch) {
    ...
    try {
      ...
      const data = await apiGet<{ vouchers: VoucherRow[]; total: number }>(`${apiPath}?${params.toString()}`);
          ...
          <table className="min-w-full border-collapse text-sm">
            <thead>
              <tr className="bg-blue-700 text-white">
                <th colSpan={6} className="px-4 py-2.5 text-left text-sm font-semibold">
                  Results for &apos;{typeLabel}&apos;
                </th>
              </tr>
              <tr className="bg-blue-600 text-white">
                {["Sr.No", "Voucher No", "Type", "Date", "Total", "Actions"].map((label) => (
                  <th
                    key={label}
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {loading ? (
                ...
                rows.map((row, index) => (
                  <tr key={row.voucherId} className="hover:bg-slate-50/80">
                    <td className="px-4 py-3 text-slate-700">{(page - 1) * pageSize + index + 1}</td>
                    <td className="px-4 py-3 font-semibold text-slate-900">{row.voucherNo}</td>
                    <td className="px-4 py-3 text-slate-700">{row.voucherType}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        <Link
                          href={`/operations/reversals?kind=${reversalKind}&documentNo=${encodeURIComponent(row.voucherNo)}`}
